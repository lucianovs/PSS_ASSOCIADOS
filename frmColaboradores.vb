...
Public Class frmColaboradores
    ...
    Private Sub frmColaboradores_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        'Carregar o Combo de Unidades
        cQuery_Unidade = "SELECT UN000_CLAUNI, UN000_NOMUNI FROM " 
        If LCase(ClassCrypt.Decrypt(g_Login)) <> "admin" Then
            cQuery_Unidade += "(EUN000 inner join EUN013 on EUN013.UN013_CODUNI=EUN000.UN000_CODRED) WHERE " & _
                "EUN013.UN013_CODUSU=" & getCodUsuario(ClassCrypt.Decrypt(g_Login)).ToString & _
                " AND UN013_PERACE > 0 and "
        Else
            cQuery_Unidade += "EUN000 WHERE "
        End If
        cQuery_Unidade += "right(UN000_CLAUNI,2)<>'00'"
        ...
        Using da As New OleDbDataAdapter()
            ...
            If dtUnidade.Rows.Count() > 0 Then
                For x = 0 To dtUnidade.Rows.Count() - 1
                    cbUnidades.Items.Add(dtUnidade.Rows(x).Item("UN000_CLAUNI") & " - " & dtUnidade.Rows(x).Item("UN000_NOMUNI"))
                Next
            Else
    ...
    Private Sub TratarObjetos()
        ...
        cbUnidades.Enabled = bAlterar And Me.Tag > 1
        ...
        If i > -1 And Not bIncluir Then
            ...
            If Not IsDBNull(dt.Rows(i).Item("UN003_CODUNI")) Then
                sClaUni = LerClasse_Unidade(dt.Rows(i).Item("UN003_CODUNI"), sNomUni)
            Else
                sClaUni = ""
                sNomUni = ""
            End If
            cbUnidades.Text = sClaUni & IIf(sClaUni = "", "", " - ") & sNomUni
        ...
        If ConectarBanco() Then
            ...
            If Validardados(cMensagem) Then
                If bIncluir Then
                    ...
                    cSql += ")"
                    cSql += " values (" & nProxCod.ToString & ",'" & txtNmColaborador.Text & "', " & LerCod_Unidade(Microsoft.VisualBasic.Left(cbUnidades.Text, 11)).ToString
                ...
                ElseIf bAlterar Then
                    cSql = "UPDATE EUN003 set UN003_NOMCOL='" & Trim(txtNmColaborador.Text) & _
                            "', UN003_CODUNI=" & LerCod_Unidade(Microsoft.VisualBasic.Left(cbUnidades.Text, 11)).ToString
